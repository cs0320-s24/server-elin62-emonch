...
public class Search {
  ...
  public void search(String[] args) {
    ...
    if (args.length < 4) {
      System.err.println(
          "Usage: java Main <path_to_csv_file> <search_value> <column_identifier> <has_header (True/False)>");
      return;
    }
      ...
      if (hasHeader) { // checks if the header boolean is set to true
        ...
        if (!columnIdentifier.equals("-")) {
          if (hasHeader && headers != null) {
            ...
            if (columnIndex == -1) {
              System.err.println(
                  "Error: please provide a valid column identifier, or '-' if n/a"); // might remove
              return;
            }
          }
        }
