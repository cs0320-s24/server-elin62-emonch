...
public class Server {
  ...
  public void run() throws IOException, FactoryFailureException {
    Search search = new Search(this.args);
    ...
    //    } else {
    //      hasHeader =
    //          Boolean.parseBoolean(headerArgument.toLowerCase()); // Safe to parse after
    // validation
    //    }
    //    try (FileReader fileReader = new FileReader(csvFilePath)) {
    //      // creates the CSVParser object for parsing the CSV file
    //      CSVParser<MyDataClass> parser = new CSVParser<>(fileReader, row -> new
    // MyDataClass(row));
    ...
    //        filteredData =
    //            data.stream()
    //                .filter(d -> d.getColumnData(idx).equals(searchValue)) // searches for the
    // exact
